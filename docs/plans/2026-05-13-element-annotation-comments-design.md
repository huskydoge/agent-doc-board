...
The reader assigns annotation targets to headings, paragraphs, list items, blockquotes, code blocks, tables, math blocks, and images. These anchors are not shown as block-level `+` controls; they are used behind the scenes so selected text can bind to a stable rendered block.
...
Existing comments appear in a `Comments` panel in the document context. Each card can jump back to its anchored block. Comments can be edited or deleted.
