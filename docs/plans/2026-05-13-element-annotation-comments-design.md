...
The reader assigns annotation targets to headings, paragraphs, list items, blockquotes, code blocks, tables, math blocks, and images. A small gutter button appears beside each target. Clicking it opens a composer for that element. If the user has selected text inside that element, the selected text is captured as the quote; otherwise the target element text is used.
